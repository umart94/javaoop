...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        //now we will use ArrayBlockingQueue instead of ArrayList
        ArrayBlockingQueue<String> buffer = new ArrayBlockingQueue<String>(6);

        //since we are using ArrayBlockingQueue , we do not need a buffer lock
        ...
        //remove bufferlock parameter from constructor and from here the call.

        MyProducer producer = new MyProducer(buffer,ThreadColor.YELLOW);
        MyConsumer consumer1 = new MyConsumer(buffer,ThreadColor.GREEN);
        MyConsumer consumer2 = new MyConsumer(buffer,ThreadColor.BLACK);
...
class MyProducer implements Runnable{
    ...
    public void run() {
        ...
        try{
            buffer.put("EOF"); // put and take methods will block when the queue is locked
        }catch(InterruptedException e){
            System.out.println("Producer was interrupted");
        }
    }
        ...
        while(true){
            synchronized (buffer){
                ...
                try{
                    ...
                    //consumer thread can be suspended here , in calling isEmpty and peek method
                    //while its suspended (not started and running), the other consumer thread can pick up and take
                    // values.

                    //solution - we have to use synchronization code when using arrayblockingqueue classes.



                    //use buffer.peek instead of buffer.get
                    if(buffer.peek().equals(EOF)){
                        System.out.println(color + "Exiting");
                        break;
                    } else {
                        System.out.println(color + "Removed "+buffer.take());//instead of remove, use buffer.take ,
                        // ArrayBlockingQueue is A fifo
                    }
                }catch(InterruptedException e){
                    System.out.println("Producer was interrupted");
                }
